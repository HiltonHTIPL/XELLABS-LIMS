"""
Bulk-import Instruments into SENAITE from an Excel (.xlsx) file.

Usage:
    python manage.py import_senaite_instruments path/to/instruments.xlsx

Expected columns (header row, case-insensitive, order doesn't matter):
    Title           (required) - Instrument name/title
    InstrumentType  (required) - Instrument Type title; auto-created in SENAITE if it doesn't exist
    Manufacturer    (required) - Manufacturer title; auto-created in SENAITE if it doesn't exist
    Supplier        (required) - Supplier title; auto-created in SENAITE if it doesn't exist
    Model           (optional)
    SerialNo        (optional)
...
from core.senaite_service import import_instrument_row
...
class Command(BaseCommand):
    help = "Bulk-import Instruments into SENAITE from an Excel file."
