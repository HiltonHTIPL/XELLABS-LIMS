...
## 0g. Development vs Testing Mode — Keyword-Triggered Frontend Switch

**Trigger words in the user's message: "development" → switch frontend to dev mode. "testing" → switch frontend to a real production build.** This lets the user say one word in a meeting/demo to flip the running app between fast-iteration mode and a true production-build check, without remembering the underlying Docker commands.

**Current baseline (confirmed 2026-07-22):** `docker-compose.yml`'s frontend `command:` is `npm run dev -- -H 0.0.0.0` — **development mode is the committed default**, not production. (This flipped as part of a PR #20 cherry-pick that same day — any older note elsewhere in this file claiming "production only, no dev override" describes a state that no longer exists; this row is the current source of truth. If it looks wrong, re-check `docker-compose.yml`'s frontend `command:` line directly before trusting either claim.)

**Mechanism — `docker-compose.override.yml`** (Docker Compose's special filename, auto-merged into every `docker compose` command with zero `-f` flags needed; gitignored, never pushed, per the user's explicit "local only" instruction):

| User says | Action |
|---|---|
| "development" | `rm -f docker-compose.override.yml` (if present) then `docker compose up -d frontend` — recreates the container using the base file's `npm run dev` command. |
| "testing" | `cp docker-compose.testing.yml docker-compose.override.yml` then `docker compose up -d --build frontend` — overrides the command to the Dockerfile's real production command (`npm run build && npm start`), so what's demoed matches what will actually ship. |

- `docker-compose.testing.yml` (project root) holds the reusable testing-mode override template — copy it to `docker-compose.override.yml` to activate, don't edit `docker-compose.override.yml` by hand from scratch each time.
- **Both `docker-compose.override.yml` and `docker-compose.testing.yml` are gitignored — never stage or push either file**, matching the user's explicit "this is only for my local, don't push it" instruction. Confirm they're absent from `git status` before any push.
- After switching, always wait for the frontend's real readiness signal before telling the user to test — dev mode logs `✓ Ready in Ns` quickly; testing mode runs the full multi-minute `npm run build` first. Don't assume readiness from the switch command alone.
- Switching modes recreates the frontend container — expect the existing "stale browser tab after a frontend restart" gotcha (documented further down this file) and tell the user to hard-refresh (`Ctrl+Shift+R`) after switching, not just reload.
...
| **[CORRECTED 2026-07-20 — the `next dev`/`--reload` rows below are now WRONG, kept struck-through for history]** Both frontend and Django are back to plain production commands, no dev overrides | Confirmed by reading the live `docker-compose.yml` (2026-07-20): the frontend service has **no `command:` override at all** — it runs the Dockerfile's own CMD (`npm run build && npm start -- -H 0.0.0.0`), a full production build on every container start. The django service's `command:` also has **no `--reload` flag** — plain `gunicorn config.wsgi:application --bind 0.0.0.0:8001 --workers 2 --timeout 120`. So **neither** `.tsx/.ts` nor `.py` changes take effect live anymore — this bit a real session: editing `BulkStoreModal.tsx`/`StorageShell.tsx` + `inventory/views.py` and re-testing in the browser silently kept serving the old code (confirmed via `docker logs xellabs-lims-frontend-1` showing `next start`, not `next dev`). **After any backend or frontend code change, run `docker compose up -d --build frontend` (and `django` if backend `.py` changed) — a plain `docker restart` is NOT enough for the frontend since `next start` serves the already-built `.next` output, not source.** Do not trust either of the two rows below without re-verifying `docker-compose.yml` first — they describe a state that no longer exists in the file, only possibly on some other branch/machine. Re-confirmed live 2026-07-22: `docker compose restart frontend` on this machine still runs the full `npm run build && npm start` CMD (~3-4 min), matching this row, not the `next dev` claim below. |
| ~~Frontend now runs `next dev` locally, NOT production build (changed 2026-07-17)~~ — **stale, see corrected row above** | ~~`docker-compose.yml`'s frontend `command:` overrides the Dockerfile's default CMD with `npm run dev -- -H 0.0.0.0`~~ |
| ~~Django backend now auto-reloads too (changed 2026-07-17)~~ — **stale, see corrected row above** | ~~`docker-compose.yml`'s django command gained a `--reload` flag on gunicorn~~ |
| (Historical) Next.js runs in **production mode** (`npm run build && npm start`) | Frontend Dockerfile CMD is `npm run build && npm start` — this is what actually runs, in both the Dockerfile itself and the current `docker-compose.yml` (no override). Do NOT use `/app/.next` anonymous volume in production (dev-only Turbopack workaround) — but `/app/.next/cache` (webpack/SWC compilation cache only, not build output) IS safe as a **named** volume (`frontend_next_cache`, added to `docker-compose.yml` 2026-07-16) and persists across every recreate. |
...
| **[FIXED — 2026-07-15]** Sample Template creation failed with `{"field": "sampletype", "message": "Wrong contained type"}` | `SampleTemplateDataGridFieldDeserializer` (`senaite-rebrand/sampletemplate_datagrid_deserializer.py`) is a Zope adapter registered for `(List, ISampleTemplate, IBrowserRequest)`. On a **POST to create** a new template, Plone's traversal context is the `sampletemplates` *folder* — not yet an `ISampleTemplate` instance — so the adapter never fires and SENAITE's default (broken, for DataGridField) deserializer handles `partitions`/`services` instead and rejects them. The adapter **does** fire correctly on PATCH to an existing object, since by then the context genuinely is an `ISampleTemplate` — confirmed live (POST scalars-only → 201, then PATCH `{partitions, services}` to the new object's own `@id` → 204, re-fetch shows both persisted correctly). (The old comment above `sampleTemplateRestBody()` in `senaite.ts` claimed this was fully fixed at the adapter level with everything persisting in "a single create POST" — that verification apparently didn't catch this specific POST-vs-PATCH traversal-context gap.) **Fixed** in `xellabs-frontend/app/lib/senaite.ts`: `createSenaiteSampleTemplate()` now splits into (1) POST with scalar fields only (title/description/sampletype/samplepoint/composite/sampling_required/auto_partition, no partitions/services) via `sampleTemplateScalarBody()`, then (2) PATCH the new object's own URL (from the POST response's `@id`) with `{partitions, services}` via `sampleTemplateGridBody()` — exact same two-step shape `patchSampleTypeExtras()` already used for SampleType's own DataGridField (`admitted_sticker_templates`). `updateSenaiteSampleTemplate()` (PATCH on an existing template) is untouched — the combined body works fine there since the adapter already fires correctly. If the grid PATCH step fails, the create still succeeds (object exists) and a `warning` surfaces to the user instead of a hard failure, matching `patchSampleTypeExtras`'s partial-failure handling. **Also discovered while verifying this**: the local dev `senaite` Docker image was stale — built *before* the commit that added this deserializer file, so the fix silently wasn't in the running container despite being correctly checked into `senaite-rebrand/Dockerfile`. Whenever a `senaite-rebrand/` file changes, `docker compose build senaite && docker compose up -d senaite` is required to actually pick it up — editing the source file alone does nothing until the image is rebuilt. |
| SENAITE `parent_path` for create calls must never hardcode `/senaite/...` | All of `xellabs-frontend/app/lib/senaite.ts`'s legacy-API create functions (clients, container types, sample containers, preservations, sample points, sample matrices, sample types, lab contacts, departments, analysis categories, analysis services, batches, worksheets) originally hardcoded parent paths like `/senaite/clients`, `/senaite/setup/containertypes`, `/senaite/bika_setup/bika_labcontacts`. This breaks the instant `SENAITE_URL` has no path component (e.g. a reverse-proxied production URL like `http://172.21.0.1:8096`, vs. local dev's `http://senaite:8080/senaite`) — `new URL(SENAITE_URL).pathname` resolves to `/` and the resulting `parent_path` (e.g. `//batches`) makes SENAITE's legacy create API crash with the opaque `"string index out of range"` on **every single create operation**, across every admin page. Fixed (2026-07-15, commits `50c39f1`/`e7b0dbc`) via a `SENAITE_SITE_PATH` constant computed once — `new URL(SENAITE_URL).pathname.replace(/\/$/, '') || '/senaite'` (falls back to `/senaite` when the URL has no path) — with every hardcoded prefix replaced by `${SENAITE_SITE_PATH}/...`. **Any new SENAITE create-API call must use `SENAITE_SITE_PATH`, never a hardcoded `/senaite/...` string** — this class of bug is invisible in local dev (where `SENAITE_URL` happens to already end in `/senaite`) and only surfaces against a proxy-style production URL. |
| `DEFAULT_TENANT_SCHEMA` must be set in **every** environment's own `.env`, not just local dev | The existing "fresh/wiped DB" entry above documents this for local dev (schema `demo`) — the exact same gap bit a real production/staging deploy (server `34.30.6.247`, 2026-07-15): `~/xellabs-lims/.env` on the server had no `DEFAULT_TENANT_SCHEMA` at all, so the frontend never sent `X-Tenant-Schema`, Django's tenant middleware defaulted to `public`, and **every** tenant-app endpoint (`/api/lims/*`, `/api/inventory/*`, `/api/instruments/*`, `/api/reports/*`) 404'd while `/api/clients/` (shared app) worked fine — same "looks like a routing bug, is actually a missing env var" trap as the local-dev version. Fixed by adding `DEFAULT_TENANT_SCHEMA=hl-01` (that server's real tenant schema name — schema names are per-deployment, not a fixed value) to the server's `.env` and recreating the frontend container. **Whenever standing up a new environment (dev, staging, prod), setting this env var to that environment's actual tenant schema name is a required step, not optional** — it will silently 404 the entire tenant-app API surface otherwise. |
| Turbopack strips `export type { X }` **re-exports** from `'use server'` files — narrower than previously documented | The existing "`'use server'` files can only export async functions" entry above says "Type exports are fine (erased at compile time)" — true for types *defined* in the file, but **not** for `export type { X } from '...'` re-exports of a type imported from elsewhere: Turbopack strips these at build time, so any consumer component importing that type from the action file fails with `"The export X was not found in module ...`" (hit in production build, 2026-07-15, for `SenaiteClientFull` from `senaite-clients.ts` and `SenaiteRefOption` from `reference-data.ts`). Fixed by removing the re-export and having every consumer import the type directly from its canonical source (`@/app/lib/senaite`) instead of via the action file. **Never re-export a type through a `'use server'` file — always import it from where it's actually defined.** |
| `StorageLocation` frontend type must carry every SENAITE tier field `StorageModal.tsx` references | Production build also failed on `Property 'site_title'/'senaite_location_type' does not exist on type 'StorageLocation'` — the type in `app/actions/storage.ts` was missing optional fields the component already used. A reminder that adding a new field reference in a component requires adding it to the corresponding action-layer type in the same change, not after. |
| Every container restart = a full ~3-4 minute Next.js production build; a TypeScript error means a crash-loop, not a quick failure | Since the frontend Dockerfile runs `npm run build && npm start`, any TS error during that build crashes the container, which Docker then restarts, which rebuilds from scratch, repeating indefinitely — multiplying total downtime far beyond the error itself. **Always run `npx tsc --noEmit` locally before pushing anything meant to deploy** — this is the single highest-leverage check to avoid a crash-loop in production. |
| Django `Test` catalog model removed entirely (2026-07-15) | Replaced by keying `Specification`/`WorksheetAssignment`/`QCSample`/`AnalysisRequest`/New Sample pricing directly off live SENAITE Analysis Services (uid + name + `Price`/`VAT` fields), removing a Django-side mirror table that needed its own manual sync. `xellabs-frontend/app/actions/tests.ts` and `/dashboard/tests` were deleted; anywhere pricing/VAT is shown now reads `SenaiteAnalysisService.Price`/`.VAT` (capitalized, raw SENAITE field names) directly, not a Django `Test` row. **Any future feature needing analysis/test data must read live SENAITE Analysis Services — do not reintroduce a Django mirror table for this.** |
| Staff user → SENAITE sync is systemic-collision-prone on the default `admin` username (2026-07-16) | Every new staff user created via `POST /api/users/` (`core/views.py UserViewSet.perform_create`) is mirrored into SENAITE by `core/tasks.py sync_staff_user_to_senaite` → `core/senaite_service.py push_staff_user` — confirmed working end-to-end live (created a real user, saw it appear in SENAITE's own Users list with the right role group, and proved actual login by POSTing SENAITE's classic login form and observing the `__ac` session cookie set only for the correct password). BUT: `.env.example`'s own suggested defaults — `DJANGO_SUPERUSER_USERNAME=admin` and SENAITE's built-in service account `SENAITE_ADMIN_USER=admin` (used by every `_session()` call in `senaite_service.py`) — collide by default on every fresh install. SENAITE's `admin` is a Zope root/emergency user, not a normal Member: it never appears in `list_senaite_users()`, yet the name is permanently reserved, so `push_staff_user`'s `POST @users` for a same-named Django user always 400s with `"login name already in use"`. This is **permanent**, not transient, so blindly retrying (the task's old `raise self.retry()` on any failure) wasted 3 retries at 60s each for something that could never succeed. Fixed: `push_staff_user` now returns `{"permanent": True}` for this specific 400, and `sync_staff_user_to_senaite` logs a `WARNING` and skips the retry instead of erroring/retrying. `ensure_superuser.py` (the platform-superadmin bootstrap, previously the ONLY user-creation path that never synced to SENAITE at all) now dispatches the same sync task on first creation — but this still no-ops for a same-named `admin` collision; it only helps deployments that pick a different `DJANGO_SUPERUSER_USERNAME`. `.env.example` now documents this explicitly next to `DJANGO_SUPERUSER_USERNAME`. **Any staff username equal to `SENAITE_ADMIN_USER` can never get its own SENAITE Member account — this is expected, not a bug to chase.** Also confirmed a pre-existing account (`xellabs-demo-admin`, created 2026-07-13 05:38) predated the sync feature's own introduction that same day at 16:20 (commit `7dee070`) — a reminder that adding a sync-on-create hook does **not** retroactively cover rows created before the hook existed; those need a one-time manual backfill (`push_staff_user(user, new_password)` after resetting the user's password, since Django only stores it hashed and the original can never be recovered). |
...
- **Every frontend table must use the shared `DataTable` component** (`xellabs-frontend/app/dashboard/_components/DataTable.tsx`, built on `@liji-table/react` — sort/search/pagination/column pin/reorder/resize/layout persistence for free) — never a hand-rolled `<table>`. Define a `DataTableColumn<T>[]` (reproducing existing cell rendering via each column's `render()`) and a `rowActions` render-prop for trailing per-row buttons, matching the pattern already used across every admin/list page (`AnalysesShell.tsx`, `SampleTypesShell.tsx`, `WorksheetDetailShell.tsx`, etc.). If a new page/table is added without DataTable, that's a bug to fix, not a style preference to skip.
...
**Before writing layout/positioning code for a new drawer, modal, or page section, check how existing sibling components (`_components/*.tsx` in other feature folders) already solve the same problem — copy their values, don't invent new ones.**
...
**[REVERSED — 2026-07-17] Drawer/modal vertical bounds: every drawer, slide-over, and centered modal in the app must sit BETWEEN the top navbar and bottom footer, never cover them.** This is the opposite of the rule that stood here previously (full-viewport `top:0, bottom:0` overlay) — the user explicitly asked for the old bounded look app-wide, after it shipped first on the new Lab Departments/Lab Contacts Administration panels. Implementation: `DashboardShell.tsx`'s root element defines two CSS custom properties — `--dashboard-header-h: 56px` (matches the header's own `h-14`) and `--dashboard-footer-h: 40px` (the footer bar's own explicit `height: 40` px, set alongside these vars so they can never drift out of sync with each other). Every drawer/modal's outermost `position: 'fixed'` overlay div uses `top: 'var(--dashboard-header-h)', bottom: 'var(--dashboard-footer-h)'` instead of `inset: 0` or `top: 0, bottom: 0` — this bounds the dimming backdrop AND the panel/dialog together in one change, since nested `position: 'absolute'` panels inherit the reduced containing block automatically. Centered modals (confirm dialogs, QR scanner, etc.) get the same treatment on their outer `alignItems:center, justifyContent:center` flex container, so the dialog naturally centers within the bounded area instead of the full viewport. **Any new drawer/modal must use these two CSS vars, never hardcoded pixel values or `inset: 0`/`top:0,bottom:0`** — grepping for `var(--dashboard-header-h)` across `_components/` shows every existing example.
...
## 16g. SENAITE Method / Manufacturer / Instrument creation — three different REST behaviors, confirmed live (2026-07-21)

Built the missing create/edit UI for SENAITE-native Method and Instrument (previously the `/dashboard/methods` and `/dashboard/instruments` top-level pages only managed unrelated **Django-local** `Method`/`Instrument` models — a full mirror with zero connection to SENAITE, confirmed by reading `app/actions/methods.ts`/`instruments.ts`, both `djangoFetch`-only). Both pages are now **tabbed**: a new SENAITE-native tab (feeds Analysis Services/Worksheets directly) alongside the pre-existing Django-local tab, renamed **"Documentation Records"** (Methods — accreditation/instructions paperwork) and **"Calibration & Maintenance"** (Instruments) to make clear the two are unrelated data sources. Root-caused each type's REST behavior empirically before writing any code (per §0f) rather than assuming one pattern fits all three:

| Type | REST behavior (confirmed live) | Implementation |
|---|---|---|
| **Method** (`bika.lims.content.method`, Archetypes, folder `methods`) | Fully clean via plain `plone.restapi` — create/read/update all work, **even with its real (non-computed) `Instruments`/`Calculations` UIDReferenceFields set**. No address-style field, so it never trips the `inline_field_validator` crash that hits Client/Calculation/LabContact/Laboratory (§16b/§16c). | Reuses the existing generic Dexterity-style CRUD trio (`createSetupItem`/`updateSetupItem`/`fetchSetupList` in `senaite-setup.ts`) verbatim — `app/actions/senaite-methods.ts`, UI `SenaiteMethodsShell.tsx` (an `AdminRefShell` instance). Zero new REST-handling code needed. |
| **Manufacturer** (Dexterity, folder `setup/manufacturers`, required by `Instrument.Manufacturer`) | Same as Supplier/InstrumentType — clean via restapi. | New Administration page, `app/actions/manufacturers.ts`, same generic helpers. |
| **Instrument** (`bika.lims.content.instrument`, Archetypes, folder `bika_setup/bika_instruments`) | **restapi is completely broken for this type, both read AND write** — any restapi call that returns a full Instrument object (GET, or a create/PATCH response body) 500s with `"No converter for making <InstrumentType ...> JSON compatible"` (the reference-field serializer can't handle it at all). Confirmed a restapi CREATE attempt actually **rolled back** — the object was never persisted, a stricter failure than AnalysisRequest's "partial write survives a bogus error" case (§16f). | Must use the **legacy v1 API exclusively** — `v1/create` + `v1/update` for writes, and the v1 **singular** endpoint (`v1/instrument/<uid>`) for full-detail reads (the v1 *list* endpoint omits detail fields, same as every other setup type, but the singular endpoint returns everything — `InstrumentType`/`Manufacturer`/`Supplier`/`Model`/`SerialNo` — as plain JSON-safe `{uid,url,api_url}` refs, no serializer crash). New `createSenaiteInstrument`/`updateSenaiteInstrument` in `senaite.ts` (next to the pre-existing `fetchSenaiteInstruments`), server actions in `app/actions/senaite-instruments.ts`. **Any future write to this type must go through these two functions — never restapi.** |

**A second, unrelated real bug found and fixed while verifying the end-to-end flow (Manufacturer → Method → Instrument → Analysis Service → Sample):** `AnalysisService.LowerLimitOfQuantification`/`UpperLimitOfQuantification` genuinely **block creation** when sent as `""` — SENAITE rejects with `"Value u'' is not floatable"` and the object is never created at all (a real, blocking failure — NOT the already-handled cosmetic `"'NoneType' object has no attribute 'form'"` response-rendering bug every other blank-optional field on this type hits, where the object still gets created underneath despite `success:false`, per `createSenaiteAnalysisService`'s existing verify-by-refetch fallback). Isolated by testing each blank float-ish field one at a time via raw API calls — every other candidate (`LowerDetectionLimit`, `UpperDetectionLimit`, `Precision`, `ExponentialFormatPrecision`, `DuplicateVariation`, `VAT`) only hit the harmless bogus error; only these two truly blocked. Fixed in `analysisServiceBody()` (`senaite.ts`) by defaulting both to `'0.00'` instead of `''`, matching the existing `Price`/`BulkPrice`/`MaxTimeAllowed` pattern for this same class of bug. **Any Analysis Service create/update that leaves both Limits-tab quantification fields blank would previously have silently failed to create the service at all** — this was invisible to a normal user since the multi-tab form (`AnalysesShell.tsx`) shows the resulting field error/toast without switching to whichever tab holds the erroring field, and a validation failure sets no toast at all (only `fieldErrors`) — worth revisiting as a separate UX fix if this recurs, not fixed here since out of scope for this task.

**Verified end-to-end via a real Playwright browser run** (not just `tsc`/build): created a Manufacturer, a Method, an Instrument (linking the Method), an Analysis Service (linking both), logged a real Sample requesting that service, and confirmed the Instrument is selectable on a Worksheet — full chain, driven through the actual UI, in `D:\Testing\xellabs-automation-test\tests\specs\full-flow-method-instrument.spec.ts`.

## 16h. `submitAnalysisResult`'s own success/failure signals were both unreliable — two false negatives, root-caused live (2026-07-22)

Found while building a full Client → Sample → Receive → Worksheet → CSV Result Import Playwright E2E test (`D:\Testing\xellabs-automation-test\tests\specs\client-to-result-import-e2e.spec.ts`). The import kept reporting `"0 submitted / 1 error"` for a result that direct SENAITE API inspection proved WAS genuinely being written — i.e. the app was treating real successes as failures. Root-caused two distinct false-negative bugs in `submitAnalysisResult()` (`app/lib/senaite.ts`), both used by every Result-submission path (Quality Result Import, Worksheets, Batches — not just the new test):

1. **The `/update/<uid>` Result-write call's own `success` flag lies for any Analysis whose Service has a blank-ish detection-limit history.** Confirmed live: `POST v1/update/<uid> {Result, InterimFields}` returns **HTTP 200** with `{success:false, message:"Value '' is not floatable"}` — yet the Result value genuinely gets written underneath (same `to_float('')` crash class as §16g's `LowerLimitOfQuantification`/`UpperLimitOfQuantification`, this time thrown while the endpoint recomputes a formatted-result summary for its own JSON response, *after* the field write already committed). The old code trusted `updateData.success === false` as ground truth and bailed immediately. Fixed: on that condition, re-fetch the Analysis and compare its actual `Result` field to what was sent — only treat it as a real failure if the value genuinely didn't land.
2. **`content_status_modify`'s HTTP response is unreliable in both directions**, not just the already-documented one. The existing comment only covered "res.ok but the transition was silently rejected" (unreceived-sample guard). The **inverse** is equally real: `res.ok` can be **false** (confirmed: a genuine 500) even though the `submit` workflow transition **did** commit — the 500 comes from the classic-Zope-view's post-transition redirect trying to *render* the Analysis's own `base_view` template, which independently crashes on the same blank-detection-limit `to_float('')` bug. Fixed: never short-circuit on a non-ok `content_status_modify` response — only the re-fetched `review_state` is trustworthy proof, in either direction.
3. **Even the re-fetched `review_state` needs a brief poll, not a single immediate read** — confirmed live: a transition that had already committed was read back as still `"assigned"` on an immediate GET, then correctly showed `"to_be_verified"` on a GET made only moments later (catalog-brain metadata lagging the ZODB commit by up to a couple of seconds). `submitAnalysisResult`'s final state-check now retries the GET up to 4 times, 1s apart, before concluding failure.

**Any future debugging of a "result submission failed" report for this codebase must verify the actual SENAITE state directly (`@@API/senaite/v1/Analysis/<uid>?complete=true`) before trusting either this endpoint's HTTP status or its own `success` field — both have been proven to independently lie in both directions.** The E2E test's own retry helper (`QualityResultImportPage.uploadAndConfirmWithRetry`) was updated to match: if a retry's preview shows the row as already-resulted/"Protected", that's the success signal, not the Done screen's submitted-count (which can legitimately be 0 on a retry whose only row already succeeded in an earlier attempt).

**Also confirmed**: running this kind of timing-sensitive Playwright flow against `npm run dev` (Turbopack) is unreliable on its own terms — a Server Action's in-flight `fetch` calls can get interrupted by a concurrent route recompile (`ECONNRESET`/`aborted` in the frontend logs correlated exactly with a `Compiling ...` log line). Confirmed the same test flow is stable back-to-back (3/3) once switched to the production build via the existing dev/testing mode toggle (§0g, `docker-compose.testing.yml`). **Any E2E test exercising multi-step server-action-heavy flows should be run in testing mode, not dev mode, if it shows intermittent step-level failures that don't reproduce on manual retry.**
